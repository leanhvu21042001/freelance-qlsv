...
// Add a computed property for the formatted date
const formattedDate = computed(() => {
  return moment(formData.dob).format("YYYY-MM-DD");
});

// You can update formData.dob when the input changes
watch(formattedDate, (newFormattedDate) => {
  formData.dob = moment(newFormattedDate, "YYYY-MM-DD").toISOString();
});

const v$ = useVuelidate(rules, formData);

const { isLoading, isError, data } = useQuery({
  queryKey: ["StudentsService.show(studentId)"],
  queryFn: () => StudentsService.show(studentId),
  retry: 0,
});

watchEffect(() => {
  formData.name = data.value?.name;
  formData.dob = data.value?.dob;
  formData.gender = data.value?.gender;
  formData.email = data.value?.email;
  formData.class_id = data.value?.class_id;
...
const { mutate } = useMutation({
  mutationKey: ["StudentsService.update"],
  mutationFn: () =>
...
<template>
  <div>
    ...
    <h2 class="text-center" v-if="isError">Student not found</h2>
